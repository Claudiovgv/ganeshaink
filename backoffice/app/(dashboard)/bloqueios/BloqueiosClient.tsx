...
import type { TimeBlock, TimeBlockConflict, TimeBlockInput, TimeBlockType } from '@/lib/types';
import Button from '@/components/Button';
import { previewTimeBlockAction, createTimeBlockAction, deleteTimeBlockAction } from '@/lib/actions';
import { formatLisbon } from '@/lib/timezone';

const emptyForm = { type: 'vacation' as TimeBlockType, reason: '', startDate: '', startTime: '09:00', endDate: '', endTime: '18:00' };
...
export default function BloqueiosClient({ initial }: { initial: TimeBlock[] }) {
  ...
  function handleCheck() {
    ...
    startTransition(async () => {
      try {
        const res = await previewTimeBlockAction(toInput());
  ...
  async function doCreate(ids: number[]) {
    const created = await createTimeBlockAction({ ...toInput(), cancelAppointmentIds: ids }) as TimeBlock;
        ...
        <div className="space-y-3">
          {blocks.map((b) => (
            <div key={b.id} className="flex items-center justify-between bg-bg-card border border-gold-border/30 rounded-lg px-4 py-3">
              <div>
                <p className="text-sm font-medium text-text-primary">{b.reason || TYPE_LABEL[b.type]}</p>
                  ...
                  {conflicts.map((c) => (
                    <label key={c.id} className="flex items-start gap-3 bg-bg-section border border-gold-border/30 rounded-lg px-3 py-2.5 cursor-pointer">
                      ...
                      <div className="flex-1 text-sm">
                        ...
                        <p className="text-text-secondary text-xs">{formatLisbon(c.startDatetime, 'dd/MM/yy HH:mm')}</p>
                      </div>
                    </label>
                  ))}
                </div>
